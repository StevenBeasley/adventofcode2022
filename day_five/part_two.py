...
def generate_stacks(starting_positions: List[Union[str, None]]):
    ...
    for i in range(len(starting_positions), -1, -1):
...
def process_instruction(stacks: List[List[str]], instruction: Tuple[int, int, int]):
    ...
    # copy the crates
    crates = stacks[start_position][-quantity:]
    # add to end position
    stacks[end_position] = [*stacks[end_position], *crates]
    # delete from original position
    stacks[start_position] = stacks[start_position][:-quantity]
